...
class LaporanProsesPencetakaniCetakController extends Controller
{
        ...
        if ($request->ajax() && $request->input('columnsData') != null) {
            ...
            if (!empty($search)) {
                ...
                $query->where(function ($q) use ($searchLower) {
                    $q
                        ...
                        ->orWhere('kuaniti_cetakan', 'like', '%' . $searchLower . '%')
                        ->orWhere('kuaniti_waste', 'like', '%' . $searchLower . '%')
            ...
            foreach ($uom as $row) {
                ...
                if ($row->status == 'checked') {
                    ...
                    <a class="dropdown-item"   id="swal-warning" data-delete="' . route('laporan_proses_pencetakani.delete', $row->id) . '">Delete</a>';
    ...
    public function approve_approve(Request $request, $id){
        ...
        $laporan_proses_pencetakani->verified_by_date = Carbon::now('Asia/Kuala_Lumpur')->format('d-m-Y h:i:s A');
        $laporan_proses_pencetakani->verified_by_user = Auth::user()->user_name;
        $laporan_proses_pencetakani->verified_by_designation = (Auth::user()->designations != null) ? Auth::user()->designations->name : 'not assign';
        $laporan_proses_pencetakani->verified_by_department = (Auth::user()->departments != null) ? Auth::user()->departments->name : 'not assign';
        $laporan_proses_pencetakani->save();
